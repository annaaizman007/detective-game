...
// voice.js play those files back instead of synthesising on the fly.
//
...
import { BOONS } from './gen.js';
import { CHARACTERS } from './characters.js';
import { CASES } from './cases/index.js';
import { allDialogueLines } from './dialogue.js';
...
export const traitFact = (traitId, valueId) =>
  `${TRAITS[traitId].label}: ${TRAITS[traitId].values.find((v) => v.id === valueId).label}.`;

export const traitPhrase = (traitId, valueId) =>
  `${TRAITS[traitId].label.toLowerCase()}, ${TRAITS[traitId].values.find((v) => v.id === valueId).label.toLowerCase()}`;
