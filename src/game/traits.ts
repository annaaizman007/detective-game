...
import type { TraitDef, TraitId, TraitValueDef } from "../types/game-types";
...
export const traitValue = (traitId: TraitId, valueId: string): TraitValueDef =>
  TRAITS[traitId].values.find((v) => v.id === valueId) as TraitValueDef;

export const traitLabel = (traitId: TraitId, valueId: string | null): string => {
  const v = valueId ? TRAITS[traitId].values.find((x) => x.id === valueId) : null;
  return v ? v.label : '—';
};
